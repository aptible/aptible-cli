...
Commands:
  ...
  aptible logs_from_archive --bucket NAME | --aws-region REGION | --stack NAAME --decryption-keys ONE [TWO] [THREE] | --download-location | --string-matches ONE [TWO] [THREE] --bucket=BUCKET --decryption-keys=one two three --region=REGION --stack=STACK  # Retrieves container logs from an S3 archive in your own AWS account. You must provide your AWS credentials via the environment variables AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY
