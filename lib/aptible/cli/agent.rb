...
module Aptible
  module CLI
    class Agent < Thor
      ...
      option :security_key, desc: 'Use Security Key authentication if available'
      def login
        security_key = options[:security_key]

        if security_key && which('u2f-host').nil?
          puts 'u2f-host is not installed. --security-key option cannot be' \
               'used.'
          return
        end

        email = options[:email] || ask_then_line('Email: ')
        ...
        begin
          ...
          if u2f && security_key
            ...
            puts 'Security Key Authentication'
            puts 'Insert your Security Key. Once it starts blinking, touch it.'

            token_options[:u2f] = Helpers::SecurityKey.authenticate(
              origin, app_id, challenge, devices
            )
          else
            token_options[:otp_token] = options[:otp_token] || ask_then_line(
              '2FA Token: '
            )
          end
